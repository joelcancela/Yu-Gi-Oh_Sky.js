...
(function (window) {
    ...
    each(['where', 'or', 'groupBy', 'select', 'pluck', 'limit', 'offset', 'order', 'uniq'], function (c) {
        JQ[c] = function (query) {
    ...
    var execOrder = function (orders, records) { //Had only one order, EDITED to add possibility to do custom ordering
        ...
        fn = this.jQ.getterFns[currentOrder.field];

        if (currentOrder.direction != 'custom') {
            if (currentOrder.direction == 'asc') {
                direction = 1;
            } else if (currentOrder.direction == 'desc') {
                direction = -1;
            }
            ...
            var ordering = {},
                sortOrder = currentOrder.custom_order,
                sortFunction = currentOrder.sort_function;
            for (var i = 0; i < sortOrder.length; i++)
                ordering[sortOrder[i]] = i;

            _records.sort(function (a, b) {
                return sortFunction(a, b, ordering);
            });
        }
    ...
    Q.order = function (criteria) {
        ...
        if (Object.keys(criteria).length > 0)
            this.criteria['order'].push({
                ...
                custom_order: criteria['custom_order'] || [],
                sort_function: criteria['sort_function']
            });
...
(function ($, window, document) {
    ...
    F.lastSearchResult = function () {
        if (this.search_text.length >= this.opts.search.start_length) { //SKY Enhanced search to make it work with sort (>=)
